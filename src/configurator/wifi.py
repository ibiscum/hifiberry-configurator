...
def get_current_connection() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.debug("Getting current WiFi connection using command: " + ' '.join(cmd))
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=5)
    ...
    try:
        ...
        if connection_exists:
            ...
            if passphrase:
                ...
                logger.debug(f"Running command: {' '.join(redacted_cmd)}")
            else:
                logger.debug(f"Running command: {' '.join(cmd)}")
        ...
        logger.debug(f"Running command for verification: {' '.join(cmd)}")
...
def main() -> int:
    ...
    if args.list_networks:
        ...
        return 0 if connect_to_wifi(ssid, passphrase, revert_on_failure) else 1
